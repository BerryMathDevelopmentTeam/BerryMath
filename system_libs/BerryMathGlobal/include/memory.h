...
namespace BerryMath {
    ...
    */
    ...
    class function {
    public:
        function() : name(""), script(""), expand(false) {}
        function(string n, string s, bool e = false) : name(n), script(s), expand(e) {}// 如果是拓展函数, script中就是所属动态链接库的地址
        value* run(script* s, std::vector<value*>&, std::map<string, value*>&);
        string nameOf() {
            return name;
        }
        string scriptOf() {
    ...
    typedef std::vector<string> (*DocFunction) ();// 一个是所有的参数列表, 一个是参数名和值
}
